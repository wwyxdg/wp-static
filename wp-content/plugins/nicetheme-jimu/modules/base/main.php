...
define( 'NC_OPTIMIZEUP_VERSION', NC_STORE_VER );
...
function nc_optimizeup_init() {
	...
	if( !defined('NC_STORE_ROOT_PATH') ){
		...
		function nc_optimizeup_enqueue_script() {
			wp_register_script('nicetheme-optimizeup', plugin_dir_url( __FILE__ ) . 'assets/main.js', array( 'jquery' ), NC_OPTIMIZEUP_VERSION);
		...
		function nc_optimizeup_enqueue_script_frontend() {
			if (!is_admin()) {
				$general_option = get_field('general', 'option');
		...
		if ($nc_option['seo_switcher'] ?? false) {
			include_once NC_OPTIMIZEUP_DIR . '/functions/seo.php';
		}
		if ($nc_option['smtp_switcher'] ?? false) {
			include_once NC_OPTIMIZEUP_DIR . '/functions/smtp.php';
		}

		if ($nc_option['general']['anti_spam_switcher'] ?? false) {
		...
		if( isset( $nc_option['open_cdn_replace'] ) && $nc_option['open_cdn_replace'] == 'enable' ){
            ...
            if( is_admin() ){
                if( $nc_option['cdn_admin_replace'] == 'enable'  ){
		...
		if( isset( $nc_option['nice-security-access'] ) && $nc_option['nice-security-access'] == 'enable' ){
            ...
            if( !preg_match('/[^a-zA-z0-9]/', $nc_option['nice-security-access-path'] ) ){
