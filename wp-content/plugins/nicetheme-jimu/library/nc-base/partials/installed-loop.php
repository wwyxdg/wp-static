                ...
                <div class="action-links">
                    <ul class="plugin-action-buttons">
						<?php if ( is_plugin_active( $key ) ) { ?>
                            <li>
                                <a class="install-now button aria-button-if-js"
                                   href="<?php echo admin_url( 'admin.php?page=nc-modules-store&deactivate=' . urlencode( $key ) . '&_wpnonce=' . wp_create_nonce( 'nc-store-deactivate-plugin-' . $key ) ); ?>"
                                   role="button">停用积木</a>
                            </li>
						<?php } else { ?>
                            <li>
                                <a class="install-now button aria-button-if-js"
                                   href="<?php echo admin_url( 'admin.php?page=nc-modules-store&activate=' . urlencode( $key ) . '&_wpnonce=' . wp_create_nonce( 'nc-store-activate-plugin-' . $key ) ); ?>"
                                   role="button">启用积木</a>
                            </li>
                            <li>
                                <a class="delete-nc-module"
                                   href="<?php echo admin_url( 'admin.php?page=nc-modules-store&delete-module=' . urlencode( $key ) . '&_wpnonce=' . wp_create_nonce( 'nc-store-delete-module-' . $key ) ); ?>">删除积木</a>
                            </li>
						<?php } ?>
                        <li>
                        </li>
