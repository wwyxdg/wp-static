...
// json save path
$config_save_paths = array(
	'default' => '默认'
);
$reg_paths         = array();
foreach ( apply_filters( 'nc_save_json_paths', array() ) as $key => $value ) {

	$path = str_replace( WP_CONTENT_DIR . '/', '', $value );

	$reg_paths[ $path ] = $path;
}

$field_group['config_save_path'] = isset( $field_group['config_save_path'] ) ? $field_group['config_save_path'] : 'default';


// 保存位置
acf_render_field_wrap( array(
	'label'        => '保存位置',
	'instructions' => '配置文件保存位置',
	'type'         => 'select',
	'name'         => 'config_save_path',
	'prefix'       => 'acf_field_group',
	'value'        => $field_group['config_save_path'],
	'choices'      => array_merge( $config_save_paths, $reg_paths )
) );
